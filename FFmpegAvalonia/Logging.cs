...
public static class Logging
{
    ...
    private static readonly ExpressionTemplate LogFileExpressionTemplate = new(LogFileTemplate);
    
    private static readonly string LogDirectory = Path.Combine(AppContext.BaseDirectory, "logs");
    
    public static void Setup(AppSettings settings)
    {
        Thread.CurrentThread.Name = "Main";
        
        var timeStamp = DateTime.Now.ToString(DateTimeFormat);
        var filePath = Path.Combine(LogDirectory, $"debug_{timeStamp}.log");
        
        Directory.CreateDirectory(LogDirectory);

        var result = Enum.TryParse(settings.Settings.LogEventLevel, out LogEventLevel logLevel);
        if (!result)
        {
            logLevel = LogEventLevel.Information;
            settings.Settings.LogEventLevel = "Information";
        }
        
        Log.Logger = new LoggerConfiguration()
            .ConfigureDefaults(filePath, logLevel)
            .CreateLogger();
        ...
        Log.Logger.CleanupLogFiles(LogDirectory, "debug*.log", settings.Settings.LogInstances);
    }

    private static void CleanupLogFiles(this ILogger logger, string location, string searchPattern, int fileInstances)
    {
    ...
    private static LoggerConfiguration ConfigureDefaults(this LoggerConfiguration loggerConfiguration, string filePath, LogEventLevel logLevel)
    {
    ...
    private static LoggerConfiguration WithMemoryUsage(this LoggerEnrichmentConfiguration loggerEnrichmentConfiguration)
    {
